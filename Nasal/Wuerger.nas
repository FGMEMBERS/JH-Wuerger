...
var enginecontrol=func{
	var eng0ff=getprop("/fdm/jsbsim/propulsion/engine[0]/fuel-flow-rate-gph");

	var fftotal=eng0ff;
	if (fftotal>0.00) {
		var hours=(getprop("/fdm/jsbsim/propulsion/total-fuel-lbs")/6.03)/fftotal;
		var range=hours*getprop("/instrumentation/gps/indicated-ground-speed-kt");
		setprop("/voodoomaster/engines/fuel_flow_total_gph", fftotal);
